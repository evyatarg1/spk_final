...
void print_eigenvalues(double * arr_eigenvalues, int n);
void wam(Matrix * adj_matrix, int n, int dim, Point ** point_arr);
void ddg(Matrix * diag_degree_mat, Matrix * adj_matrix, int n);
void lnorm(Matrix * l_norm_mat, Matrix * adj_matrix, Matrix* sqrt_diag_degree_mat, int n);
void jacobi(Point ** point_arr, Matrix * a_eigenvalues, Matrix * v_eigenvectors,  double * arr_eigenvalues, Eigen ** final_eigen, int n);
void spk();
void free_mat(Matrix * mat);
void free_point(Point ** point_arr, int n);
void free_eigen(Eigen ** final_eigen, int n);
void main_logic(int k, char * goal, char * f_name, int flag);  
...
void main_logic(int k, char * goal, char * f_name, int flag)
{
    ...
    */
    ...
    assert(final_eigen != NULL);
    mat_to_eigen_struct(a_eigenvalues, v_eigenvectors, final_eigen);
    sort_eigen_values(final_eigen);
    */
      
    wam(adj_matrix, n, dim, point_arr);
    
    if(strcmp(goal,"wam")==0)
    {   
        print_mat(adj_matrix);
        free_mat(adj_matrix);
        free_point(point_arr, n);
        return;
    }
    
    ddg(diag_degree_mat, adj_matrix, n);
    if(strcmp(goal,"ddg")==0)
    {
        ddg(diag_degree_mat, adj_matrix, n);
        print_mat(diag_degree_mat);
        free_mat(adj_matrix);
        free_mat(diag_degree_mat);
        free_point(point_arr, n);
        return;
    }
    
    copy_mat_a_to_b(diag_degree_mat, sqrt_diag_degree_mat); /*coping D */
    lnorm(l_norm_mat, adj_matrix, sqrt_diag_degree_mat, n);
    if(strcmp(goal,"lnorm")==0)
    {
        print_mat(l_norm_mat);
        free_mat(adj_matrix);
        free_mat(diag_degree_mat);
        free_mat(sqrt_diag_degree_mat);
        free_mat(l_norm_mat);
        free_point(point_arr, n);
        return;
    }

     if(strcmp(goal,"jacobi")==0 || strcmp(goal,"spk")==0)
   {
        jacobi(point_arr, a_eigenvalues, v_eigenvectors, arr_eigenvalues, final_eigen, n);
        if(strcmp(goal,"jacobi")==0)
        {
            print_eigenvalues(arr_eigenvalues, n);
            printf("\n");
        }
        ...
        free_point(point_arr, n);
        free_mat(a_eigenvalues);
        free_mat(v_eigenvectors);
        free_eigen(final_eigen, n);
...
int main(int argc, char** argv)
{
    ...
    goal = argv[2];
    f_name = argv[3];
    if(strcmp(goal,"wam")!=0 && strcmp(goal,"ddg")!=0 && strcmp(goal,"lnorm")!=0 && strcmp(goal,"jacobi")!=0 && strcmp(goal,"spk")!=0)
    {
        printf("Invalid Input!\n");
        exit(1);
    }
